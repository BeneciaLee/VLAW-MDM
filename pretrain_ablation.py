...
def get_config(mname = "facebook/blenderbot-400M-distill"):
    ...
    # config.task_list = ['mlm'] # MLM
    # config.task_list = ['mlm', 'cid']  # MLM, CID
    # config.task_list = ['mlm', 'cid', 'gcd']  # MLM, CID, GCD
...
def pretrain(args):
    ...
    ###########################################################################
    ###########################################################################
    dirpath = f"../checkpoints/ablation/mlm_cid/"
    ...
    #     save_last=True,
    #     save_top_k=2,
    #     filename='{epoch}-{mlm_loss:.2f}{mlm_perplexity:.2f}',
    #     verbose=True,
    #     monitor='mlm_loss',
    #     mode='min',
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_last=True,
        save_top_k=1,
        filename='{epoch}-{mlm_loss:.2f}{mlm_perplexity:.2f}{cid_loss:.2f}{cid_perplexity:.2f}{gcd_loss:.2f}{gcd_perplexity:.2f}',
        verbose=True,
        monitor='mlm_loss',
        mode='min',
        save_on_train_epoch_end=True
    ...
    trainer = pl.Trainer(max_epochs=args.max_epochs, accelerator="gpu", precision=16,
                         accumulate_grad_batches=(2024//config.batch_size), amp_backend="native", callbacks=[checkpoint_callback])
